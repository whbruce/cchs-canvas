...
reporter = Reporter()
print("=== Assignments due on %s ====" % (args.date.strftime("%m/%d")))
...
print("\n==== Missing assignments ====")
for status in status_list:
    print("%-8s: %-20.20s %s" % (status.course, status.name, status.due_date.strftime("%m/%d")))
print("\n==== Assignments with low score ====")
status_list = reporter.run_assignment_report(SubmissionStatus.Low_Score)
for status in status_list:
    print("%-8s: %-20.20s %s [%d%%]" % (status.course, status.name, status.due_date.strftime("%m/%d"), status.score))
